...
# Windows-safe env for ``@patch.dict(..., clear=True)`` tests: the decorator
# wipes the whole environment, and on Windows ``pathlib.Path.home()`` raises
# "Could not determine home directory" when USERPROFILE / HOME /
# HOMEDRIVE+HOMEPATH are all absent (POSIX falls back to the ``pwd`` module,
# which is why these tests only broke on Windows).  ``get_hermes_home()``
# reads HERMES_HOME first, then LOCALAPPDATA, then Path.home().  Preserving
# these variables keeps the "no Feishu env vars" semantics of clear=True
# while letting home resolution work on every platform.
_HOME_ENV = {
    k: v
    for k, v in os.environ.items()
    if k in ("HERMES_HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "HOME", "LOCALAPPDATA")
    and v
}
...
class TestConfigEnvOverrides(unittest.TestCase):
    @patch.dict(os.environ, {**_HOME_ENV, 
...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    @patch.dict(os.environ, {**_HOME_ENV, }, clear=True)
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_GROUP_POLICY": "open"}, clear=True)
    ...
    @patch.dict(
        os.environ,
        {**_HOME_ENV, "FEISHU_GROUP_POLICY": "allowlist", "FEISHU_ALLOWED_USERS": "ou_allowed", "FEISHU_BOT_NAME": "Hermes Bot"},
        clear=True,
    )
    ...
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_VERIFICATION_TOKEN": "expected-token"}, clear=True)
    ...
    @patch.dict(
        os.environ,
        {**_HOME_ENV, "HERMES_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2"},
        clear=True,
    )
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {**_HOME_ENV, "HERMES_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {**_HOME_ENV, "HERMES_HOME": temp_home}, clear=False):
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {**_HOME_ENV, "FEISHU_BOT_OPEN_ID": "ou_env", "FEISHU_BOT_NAME": "Env Hermes"},
        clear=True,
    )
    ...
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_BOT_OPEN_ID": "ou_env"}, clear=True)
...
class TestWebhookSecurity(unittest.TestCase):
    ...
    def _make_adapter(self, encrypt_key: str = "") -> "FeishuAdapter":
        ...
        with patch.dict(os.environ, {**_HOME_ENV, "FEISHU_APP_ID": "cli", "FEISHU_APP_SECRET": "sec", "FEISHU_ENCRYPT_KEY": encrypt_key}, clear=True):
            return FeishuAdapter(PlatformConfig())
    ...
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_ENCRYPT_KEY": "secret"}, clear=True)
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {**_HOME_ENV, "HERMES_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
...
class TestGroupMentionAtAll(unittest.TestCase):
    ...
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_GROUP_POLICY": "allowlist", "FEISHU_ALLOWED_USERS": "ou_allowed"}, clear=True)
...
class TestProcessingReactions(unittest.TestCase):
    ...
    # ------------------------------------------------------------- env toggle
    @patch.dict(os.environ, {**_HOME_ENV, "FEISHU_REACTIONS": "false"}, clear=True)
