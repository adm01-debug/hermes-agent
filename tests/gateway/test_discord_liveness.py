...
async def test_liveness_probe_reports_gateway_health_failure_reason(monkeypatch, health, expected_reason):
    # max_ack_age is generous (60s) so the latency case cannot race the
    # ack window: the ack is created at bot-construction time, and on a
    # loaded machine connect() can take >1s, which made ack_stale win
    # over latency_non_finite (flaky under the full suite).
    adapter = _make_adapter(monkeypatch, interval=0.005, threshold=1, max_ack_age=60.0)
