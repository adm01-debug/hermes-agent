...
class TestEditMessageBlocks:
    ...
    async def test_slack_api_error_on_edit_is_not_retryable(self):
        ...
        # The gateway suite registers a MagicMock ``slack_sdk`` in sys.modules
        # at collection time (see test_send_multiple_images.py / test_slack.py)
        # whenever the real package is not installed.  ``importorskip`` cannot
        # detect that mock — it imports fine — so a mock SlackApiError would
        # silently run the test with wrong semantics (AsyncMock side_effect
        # CALLS a Mock instead of raising).  Skip unless the real module is
        # present.
        if not isinstance(errors_mod, ModuleType):
            pytest.skip(
                "real slack_sdk required — only a collection-time mock is "
