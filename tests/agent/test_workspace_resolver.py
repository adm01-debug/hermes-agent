...
class TestSafeDirName:
    ...
    def test_empty(self):
        # Review fix: empty names are rejected (would escape the parent dir)
        assert _safe_dir_name("") == "_invalid"

    def test_special_chars(self):
        assert _safe_dir_name("abc-123") == "abc-123"

    def test_rejects_separators(self):
        assert _safe_dir_name("../evil") == "_invalid"
        assert _safe_dir_name("a/b") == "_invalid"
        assert _safe_dir_name("a\\b") == "_invalid"
    ...
    def test_rejects_traversal_segments(self):
        ...
        assert _safe_dir_name(".") == "_invalid"
        assert _safe_dir_name("a..b") == "_invalid"
        assert _safe_dir_name("a/../../etc") == "_invalid"

    def test_rejects_absolute_escape(self):
        assert _safe_dir_name("/etc/passwd") == "_invalid"
        assert _safe_dir_name("C:/Windows") == "_invalid"
        assert _safe_dir_name("C:\\Windows") == "_invalid"
...
# ── Review fix: workspace dir hardening (never escape <home>/workspaces) ─────

class TestWorkspaceDirHardening:
    def test_valid_name(self, temp_hermes_home: Path):
        from agent.workspace_resolver import _workspace_dir

        result = _workspace_dir(temp_hermes_home, "news-feed")
        assert result is not None
        assert result == (temp_hermes_home / "workspaces" / "news-feed")

    def test_traversal_rejected(self, temp_hermes_home: Path):
        from agent.workspace_resolver import _workspace_dir

        assert _workspace_dir(temp_hermes_home, "../evil") is None
        assert _workspace_dir(temp_hermes_home, "..") is None
        assert _workspace_dir(temp_hermes_home, "a/b") is None
        assert _workspace_dir(temp_hermes_home, "a\\b") is None

    def test_absolute_rejected(self, temp_hermes_home: Path):
        from agent.workspace_resolver import _workspace_dir

        assert _workspace_dir(temp_hermes_home, "/etc/passwd") is None
        assert _workspace_dir(temp_hermes_home, "C:/Windows") is None

    def test_evil_content_resolution_is_empty(self, temp_hermes_home: Path):
        """Traversal names in topics.yaml must resolve to nothing, not read files."""
        from agent.workspace_resolver import _resolve_workspace_content

        result = _resolve_workspace_content(temp_hermes_home, "../../secret")
        assert result == WorkspaceResult(None, None, None)


# ── Review fix: shared resolution via apply_workspace_to_event ────────────────

class TestApplyWorkspaceToEvent:
    def _make_event(self, platform="telegram", chat_id="-1001", thread_id="42"):
    ...
    def test_applies_prompt_skills_model(self, temp_hermes_home: Path):
        from agent.workspace_resolver import apply_workspace_to_event

        tp = temp_hermes_home / "platforms" / "telegram" / "_-1001" / "topics.yaml"
        tp.parent.mkdir(parents=True)
        import yaml
        tp.write_text(yaml.safe_dump({"topics": {"42": "news-feed"}}), encoding="utf-8")
        sp = temp_hermes_home / "workspaces" / "news-feed" / "SYSTEM.md"
        sp.parent.mkdir(parents=True)
        sp.write_text(
            "---\nskills:\n  - telegram-summary-bot\nmodel: google/gemma-4-31b-it\n---\nHebrew news.",
        ...
        applied = apply_workspace_to_event(event)
        assert applied is True
        assert event.channel_prompt == "Hebrew news."
        assert event.auto_skill == ["telegram-summary-bot"]
        assert event.workspace_model == "google/gemma-4-31b-it"

    def test_idempotent(self, temp_hermes_home: Path):
        from agent.workspace_resolver import apply_workspace_to_event
        ...
        event = self._make_event()
        assert apply_workspace_to_event(event) is True
        # Second call must be a no-op (already applied)
        assert apply_workspace_to_event(event) is False

    def test_no_workspace_no_change(self, temp_hermes_home: Path):
        from agent.workspace_resolver import apply_workspace_to_event
        ...
        assert event.workspace_model is None

    def test_merges_workspace_skills_with_existing(self, temp_hermes_home: Path):
        from agent.workspace_resolver import apply_workspace_to_event
        ...
        event = self._make_event()
        event.auto_skill = "config-skill"
        apply_workspace_to_event(event)
        assert "ws-skill" in event.auto_skill
        assert "config-skill" in event.auto_skill
