...
class TestRecordAuxiliaryUsage:
    ...
    def test_usage_against_missing_session_is_safe_noop(self, db):
        """Anti-resurrection contract (N1): recording usage for a session that does
        not exist must NOT fail (FK safety) and must NOT create a session row
        (a deleted session must stay deleted — token accounting must never
        resurrect it)."""
