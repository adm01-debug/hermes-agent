...
function Write-PathDiag {
    ...
    # anything parsing it.
    #
    # $host.UI.WriteErrorLine rather than [Console]::Error.WriteLine: this file
    # sets [Console]::OutputEncoding up top (see the UTF-8 block), which on
    # Windows re-creates the Console streams when the handles are redirected and
    # silently drops writes made through them. The engine's own stderr channel
    # is unaffected.
    param([string]$Message)
    try {
        $host.UI.WriteErrorLine("[hermes] $Message")
    } catch {
