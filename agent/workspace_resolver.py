...
def get_workspace_skill_dirs(
    ...
    if not workspace_name:
        return []
    ws_dir = _workspace_dir(home, workspace_name)
    if ws_dir is None:
        return []
...
def apply_workspace_to_event(event: Any) -> bool:
    """Resolve workspace prompt/skills/model from ``event.source`` and apply.

    Shared resolution path (review fix): called from the gateway message
    handler for EVERY platform (Telegram, Discord, Slack, CLI, TUI, ...), so
    workspace context is not Telegram-only.  Idempotent — events already
    resolved (``_workspace_applied`` flag) are skipped.

    Priority (matching the folder-resolver contract):
      workspace prompt  >  platform topic/channel prompt already on the event
      workspace skills  >  (merged with) existing auto_skill
      workspace model   >  (overrides config default; /model session override
                           still wins — enforced in run.py)

    Returns True when a workspace was applied.
    """
    if getattr(event, "_workspace_applied", False):
        return False
    ...
    if platform is None:
        return False
    platform_slug = platform.value if hasattr(platform, "value") else str(platform)
    ...
    ws = resolve_workspace(platform_slug, chat_id, thread_id_str)
    if not (ws.prompt or ws.skills or ws.model):
        event._workspace_applied = True  # type: ignore[attr-defined]
        return False

    if ws.prompt:
        event.channel_prompt = ws.prompt
    if ws.skills:
        ...
        if isinstance(current, str):
            ...
            for s in current:
                if s not in merged:
                    merged.append(s)
        event.auto_skill = merged
    if ws.model:
        event.workspace_model = ws.model
    event._workspace_applied = True  # type: ignore[attr-defined]
    return True
...
def _safe_dir_name(value: str) -> str:
    """Escape/validate a chat id or workspace name for use as a directory name.

    Rejects path separators, traversal segments, and NUL (review fix: a name
    coming from topics.yaml or user input must never escape the parent
    directory).  A leading minus becomes escaped (e.g. "-1003" → "_-1003")
    so ``Path`` doesn't interpret it as a relative-segment trick.  Returns
    "_invalid" for names that cannot be used as a single directory segment.
    """
    if not value:
        return "_invalid"
    if value in (".", ".."):
...
def _workspace_dir(home: Path, workspace_name: str) -> Optional[Path]:
    """Return the validated workspace dir under ``<home>/workspaces`` or None.

    Double-checks (defense in depth) that the resolved path still lives under
    ``<home>/workspaces`` before callers read SYSTEM.md from it.
    """
    safe = _safe_dir_name(workspace_name)
    if safe == "_invalid":
        return None
    base = (home / "workspaces").resolve()
    candidate = (home / "workspaces" / safe).resolve()
    if candidate == base or not str(candidate).startswith(str(base) + os.sep):
        return None
    return candidate
